...
def propose_theta(theta: np.ndarray, k: float) -> np.ndarray:
    ...
    # sigma
    theta_prime[2] = stats.gamma(a=previous_sigma*k*OMEGA, scale=k*(1/OMEGA)).rvs(1)
...
def log_prior(theta) -> np.ndarray[tuple[int, ...], np.dtype[np.float64]]:
    ...
    log_prior = stats.norm.logpdf(a, loc=0, scale=1)
    log_prior += stats.norm.logpdf(b, loc=0, scale=1) 
...
def metropolis_hastings(data: np.ndarray, k: float, num_iterations: int) -> np.ndarray[tuple[int, ...], np.dtype[np.float64]]:
    ...
    for i in range(num_iterations):
        ...
        theta_prime = propose_theta(thetas[-1,:], k)

        log_posterior_theta_prime = log_posterior(data, theta_prime)
        log_posterior_theta = log_posterior(data, thetas[-1,:])
        
        # instead of dividing we are subtracting the log values
        acceptance_ratio = log_posterior_theta_prime - log_posterior_theta
        
        # exponentiate the log acceptance ratio to obtain probability
        if np.random.rand() < np.exp(acceptance_ratio):
